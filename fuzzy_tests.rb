...
def exec_java(result_folder, cluster, base, base_path, m_value, error_value)
  ...
  fuzzy_jar_cmd = ( "./clustering #{method} -o #{result_folder} -v -m #{m_value} -e #{error_value} -c #{$bases_class[base]} -r 5 -a #{$pfcm_a} -b #{$pfcm_b} -n #{$fuzzyfication_n} #{$verbose_cmd} < #{base_path}" )
